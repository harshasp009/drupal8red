...
(function ($) {
  ...
  $(document).on('hover', 'a[data-target="dropdown-button"]', function (e) {
    e.preventDefault();
    e.stopPropagation();
    $(this).parent().find('button').trigger('hover');
  });

  // Handle buttons that used to be dropbutton links.
  // @see \Drupal\bootstrap\Plugin\Preprocess\BootstrapDropdown::preprocessLinks
  $(document).on('hover', '.btn-group button[data-url], .dropdown button[data-url]', function (e) {
